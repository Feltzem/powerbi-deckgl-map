...
A ready-made example is in [`samples/animation/wkp_road_prisms_sample.csv`](samples/animation/wkp_road_prisms_sample.csv) (generated by [`scripts/gen-wkp-road-prisms.ts`](scripts/gen-wkp-road-prisms.ts)). Its columns are `geometry_id, layer_type, wkp, parking_type, polygon_fill, polygon_extrude_elevation, valid_from, valid_to, tooltip_html`. Import it and bind `geometry_id`, `layer_type`, `wkp`, `parking_type` (to `Polygon fill`, as `First parking_type`), `polygon_extrude_elevation`, and `tooltip_html` (as `First tooltip_html`). The `valid_from` / `valid_to` columns are authoring-time inputs only — the visual never reads them, so leave them out of the field well.
...
A ready-made example is in [`samples/animation/wkp_time_animation_sample.csv`](samples/animation/wkp_time_animation_sample.csv) (generated by [`scripts/gen-wkp-testdata.ts`](scripts/gen-wkp-testdata.ts)): a dense timestamped scatter plus 3D-WKP paths and prisms. Its key columns are `geometry_id, layer_type, wkp, point1_latitude, point1_longitude, timestamp, scatter_radius, scatter_fill, tooltip_html`. For an obvious demo bind `timestamp`, set `Trail length` to `120`, `Animation speed` to `60`, turn on `Loop`, then tilt the map and set `Max height` to `1500` to bring the time-rug in.
